...
function sb_admin_assets($hook)
{
    ...
    if ($icon_lib !== '') {
        wp_enqueue_style('sb-icon-library', $icon_lib, [], SB_VER);
    }
 ...
 */
function sb_toggle_field($name, $label, $checked, $hint = '')
{
 ...
 */
function sb_empty_state($dashicon, $title, $hint = '')
{
...
═══════════════════════════════════════════════════════════════ */
function sb_wrap_open($title, $active_tab = '', $subtitle = '')
{
    $tabs = [
    ...
    echo '<span class="sb-topbar-ver">v' . esc_html(SB_VER) . '</span>';
    echo '</div>';
...
function sb_page_bookings()
{
    ...
    sb_wrap_open('All Bookings', 'sb-bookings', __('Track and manage every session booking in one place.', 'snapbook'));
    ...
    if (empty($bookings)) {
        echo sb_empty_state('dashicons-clipboard', __('No bookings found', 'snapbook'), __('New bookings appear here as soon as customers complete the booking form.', 'snapbook')); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped inside sb_empty_state.
    } else {
...
function sb_page_sessions()
{
    ...
    sb_wrap_open('Session Types', 'sb-sessions', __('Define the photography session types customers can choose.', 'snapbook'));
    ...
    echo '<div class="fpb-field"><label>Name <span class="req">*</span></label><input class="regular-text" type="text" id="fpb-session-name" name="name" required placeholder="Holiday / Couple Photoshoot" value="' . esc_attr($edit_row->name ?? '') . '">';
    echo '<p class="description">' . esc_html__('Shown as a tab at the top of the booking form.', 'snapbook') . '</p></div>';
    echo '<div class="fpb-field"><label>Slug <span class="req">*</span></label><input class="regular-text" type="text" id="fpb-session-slug" name="slug" required placeholder="photo" value="' . esc_attr($edit_row->slug ?? '') . '">';
    echo '<p class="description">' . esc_html__('Unique identifier — filled automatically from the name.', 'snapbook') . '</p></div>';
    echo '<div class="fpb-field"><label>Emoji / Icon</label><input class="regular-text" type="text" name="emoji" maxlength="100" placeholder="📷 or fa-solid fa-camera" value="' . esc_attr($edit_row->emoji ?? '') . '">';
    echo '<p class="description">' . esc_html__('An emoji, or a Font Awesome class like "fa-solid fa-camera" (free icons: fontawesome.com/search?o=r&m=free).', 'snapbook') . '</p></div>';
    ...
    echo sb_toggle_field('active', __('Active', 'snapbook'), isset($edit_row->active) ? (int) $edit_row->active === 1 : true, __('Visible on the booking form', 'snapbook')); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped inside sb_toggle_field.
    echo '</div>';
    ...
    if (empty($sessions)) {
        echo sb_empty_state('dashicons-category', __('No session types yet', 'snapbook'), __('Add your first session type with the form above.', 'snapbook')); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped inside sb_empty_state.
    } else {
        ...
        foreach ($sessions as $row) {
            echo '<tr>';
            echo '<td>' . sb_icon_html($row->emoji) . '</td>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped inside sb_icon_html.
...
function sb_page_packages()
{
    ...
    sb_wrap_open('Packages', 'sb-packages', __('Create the packages offered under each session type.', 'snapbook'));
    ...
    foreach ($sessions as $s) {
        ...
        echo '<option value="' . (int) $s->id . '"' . $sel . '>' . esc_html(trim(sb_icon_text($s->emoji) . ' ' . $s->name)) . '</option>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
    }
    echo '</select></div>';
    ...
    echo '</div>';
    echo '<div class="fpb-form-switches">';
    echo sb_toggle_field('featured', __('Featured', 'snapbook'), isset($edit_row->featured) && (int) $edit_row->featured === 1, __('Highlighted with a ★ Popular tag', 'snapbook')); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped inside sb_toggle_field.
    echo sb_toggle_field('active', __('Active', 'snapbook'), isset($edit_row->active) ? (int) $edit_row->active === 1 : true, __('Available for booking', 'snapbook')); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped inside sb_toggle_field.
    echo '</div>';
    echo '<div class="fpb-form-actions">';
    ...
    foreach ($sessions as $s) {
        echo '<li><a href="' . esc_url(admin_url('admin.php?page=sb-packages&session=' . (int) $s->id)) . '" class="fpb-filter-btn' . ($filter === (int) $s->id ? ' current active' : '') . '">' . sb_icon_html($s->emoji) . ' ' . esc_html($s->name) . '</a></li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped inside sb_icon_html.
    }
    echo '</ul><br class="clear" />';
    if (empty($packages)) {
        echo sb_empty_state('dashicons-archive', __('No packages yet', 'snapbook'), __('Create your first package with the form above.', 'snapbook')); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped inside sb_empty_state.
    } else {
        ...
        foreach ($packages as $row) {
            echo '<tr>';
            echo '<td>' . sb_icon_html($row->semoji) . ' ' . esc_html($row->sname) . '</td>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped inside sb_icon_html.
            echo '<td>' . esc_html($row->name) . '</td>';
            echo '<td>' . esc_html($cur) . esc_html(number_format((float) $row->price, 0)) . '</td>';
            echo '<td>' . esc_html($row->duration) . '</td>';
            echo '<td>' . ($row->featured ? '⭐' : '—') . '</td>';
            echo '<td>' . ($row->active ? '✅' : '❌') . '</td>';
            $share_url = sb_package_share_link($row, $booking_url);
            echo '<td>';
...
function sb_page_addons()
{
    ...
    sb_wrap_open('Add-ons', 'sb-addons', __('Optional extras customers can add to any package.', 'snapbook'));
    ...
    echo '<input type="hidden" name="id" value="' . ($edit_row ? (int) $edit_row->id : 0) . '">';
    echo '<div class="fpb-form-grid cols-2">';
    echo '<div class="fpb-field"><label>Name <span class="req">*</span></label><input class="regular-text" type="text" name="name" required placeholder="Drone aerial session" value="' . esc_attr($edit_row->name ?? '') . '">';
    echo '<p class="description">' . esc_html__('Shown on the add-on card in step 2 of the booking form.', 'snapbook') . '</p></div>';
    echo '<div class="fpb-field"><label>Price (' . esc_html($cur) . ') <span class="req">*</span></label><input class="small-text" type="number" name="price" required step="0.01" min="0" placeholder="150" value="' . esc_attr($edit_row->price ?? '') . '">';
    echo '<p class="description">' . esc_html__('Added on top of the package price.', 'snapbook') . '</p></div>';
    echo '<div class="fpb-field"><label>Emoji / Icon</label><input class="regular-text" type="text" name="emoji" maxlength="100" placeholder="🚁 or fa-solid fa-helicopter" value="' . esc_attr($edit_row->emoji ?? '') . '">';
    echo '<p class="description">' . esc_html__('An emoji, or a Font Awesome class like "fa-solid fa-helicopter".', 'snapbook') . '</p></div>';
    echo '<div class="fpb-field"><label>Sort Order</label><input class="small-text" type="number" name="sort_order" value="' . esc_attr($edit_row->sort_order ?? 0) . '" min="0">';
    echo '<p class="description">' . esc_html__('Lower numbers appear first.', 'snapbook') . '</p></div>';
    echo '<div class="fpb-field fpb-field-editor"><label>Description</label>';
    wp_editor(
    ...
    foreach ($all_packages as $pkg) {
        ...
        echo '<label class="fpb-pkg-check"><input type="checkbox" name="package_ids[]" value="' . (int) $pkg->id . '"' . $chk . '> ' . esc_html(trim(sb_icon_text($pkg->semoji) . ' ' . $pkg->sname . ' › ' . $pkg->name)) . '</label>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
    }
    echo '</div>';
    ...
    if (empty($addons)) {
        echo sb_empty_state('dashicons-star-filled', __('No add-ons yet', 'snapbook'), __('Create optional extras customers can add to their booking.', 'snapbook')); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped inside sb_empty_state.
    } else {
        $pkg_name_lookup = [];
        foreach ($all_packages as $pkg) {
...
{
    if (! current_user_can('manage_options')) return;
    sb_wrap_open('Date Slots', 'sb-dates', __('Control which days are open for new bookings.', 'snapbook'));
?>
...
function sb_page_settings()
{
    ...
    sb_wrap_open('Settings', 'sb-settings', __('Configure checkout, payments, notifications, and form text.', 'snapbook'));
    ...
    echo '<button type="button" class="button button-secondary" onclick="fpbCopyShortcode()">' . esc_html__('Copy', 'snapbook') . '</button>';
    echo '</div>';
    ...
    // ── Appearance ─
    $theme_defaults = function_exists('sb_default_theme_colors') ? sb_default_theme_colors() : ['primary' => '#b8956a', 'accent' => '#3d6b78'];
    ...
    $detected_url = sb_get_booking_page_url();
    if ($detected_url !== '') {
    ...
    $checkout_mode = function_exists('sb_get_checkout_mode') ? sb_get_checkout_mode() : 'direct';
    $cf_catalog    = function_exists('sb_checkout_field_catalog') ? sb_checkout_field_catalog() : [];
    $cf_fields     = function_exists('sb_get_checkout_form_fields') ? sb_get_checkout_form_fields() : [];
    ...
    $ccf_types  = function_exists('sb_custom_checkout_field_types') ? sb_custom_checkout_field_types() : [];
    $ccf_fields = function_exists('sb_get_custom_checkout_fields') ? sb_get_custom_checkout_fields() : [];
